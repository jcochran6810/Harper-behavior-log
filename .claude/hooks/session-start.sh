...
#   - Runs npm install, typecheck and tests for a known-good baseline.
...
# True when package.json defines the named npm script.
has_script() {
  [ -f package.json ] || return 1
  python3 -c 'import json,sys
try:
    print("yes" if sys.argv[1] in json.load(open("package.json")).get("scripts", {}) else "no")
except Exception:
    print("no")' "$1" 2>/dev/null | grep -q yes
}
...
(Local environment — automatic git sync + npm install/typecheck/test are
...
#   - On `main` directly → BLOCKER (main is the live production branch).
SYNC_STATUS=""
DRIFT_BLOCKER=""
...
if [ -z "$CURRENT_BRANCH" ]; then
  ...
  SYNC_STATUS="⛔ On main directly — STOP. main is the live production branch; sessions belong on a per-session branch. Create one from origin/main: \`git fetch origin && git checkout -b claude/<slug> origin/main\`."
  DRIFT_BLOCKER="yes"
...
if ! has_script typecheck; then
  TYPECHECK_STATUS="⏭  npm run typecheck (no such script — skipped)"
elif npm run typecheck >&2 2>&1; then
  TYPECHECK_STATUS="✅ npm run typecheck"
else
  TYPECHECK_STATUS="‼️ npm run typecheck FAILED"
fi

# `npm run lint` is intentionally NOT run — it can prompt interactively for
# ESLint config on first run and would hang the hook. Wire it back in once
# linting is configured.

log "npm test..."
if ! has_script test; then
  TEST_STATUS="⏭  npm test (no test suite yet — skipped)"
elif npm test --silent >&2 2>&1; then
...
Reminder: \`main\` is the live production branch (Vercel auto-deploys on push). All work happens on \`$CURRENT_BRANCH\` and only goes live when the user types \"end session\" and CLAUDE.md's end-session protocol merges this branch into main.
